...
const Interview = ({ params }) => {
  ...
  const GetInterviewDetails = async () => {
    try {
        console.log("Fetching interview details for ID:", params.interviewId); // Debugging
        console.log("Mock",MockInterview);
        //console.log("ID",MockInterview.mockid);
        const result = await db
                .select()
                .from(MockInterview)
                .where(eq(MockInterview.mockid, params.interviewId))
              console.log("Result-GetInterviewDetails",result[0]);
